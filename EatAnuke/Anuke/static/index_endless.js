...
function gameTapEvent(e) {
    ...
    if ((p.id == tar.id && tar.notEmpty) || (p.cell == 0 && x < blockSize) || (p.cell == 1 && x > blockSize && x < 2 *
        ...
        _gameTimeNum = _gameTimeNum * 0.5 - 1.5;
    }
    return false;
}
...
function shareText(score) {
    ...
    SubmitResults();
    if (score <= 99) return 'Anuke:No';
    if (score <= 149) return 'Anuke:Yes';
    if (score <= 199) return 'Anuke:Well';
    if (score <= 250) return 'Anuke:Quite good';
    return 'Anuke:Ohno';
}
